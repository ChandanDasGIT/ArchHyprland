...
Item {
    ...
    RowLayout {
        ...
        // ================= 1. custom/toggle-notes =====================
        Segment {
            icon: "\uf0ca"
            tooltip: "TO-DO list and SHORTCUTS"
            onClicked: toggleNotesProc.running = true
        }
        Process {
            id: toggleNotesProc
            command: ["bash", "-c", "$HOME/.config/waybar/toggle_notes.sh"]
        }

        // ================= 2. group/hardware (drawer) ==================
        Segment {
            id: drawerToggle
            icon: hwDrawer.open ? "\u276f" : "\u276e" // ❯ / ❮
        ...
        Item {
            ...
            RowLayout {
                ...
                Segment {
                    icon: "\uf249"
                    tooltip: "Edit TO-DO list"
                    onClicked: todoProc.running = true
                }
                Process {
                    id: todoProc
                    command: ["bash", "-c", "kitty --class floating-todo -e micro $HOME/.config/waybar/to-do.txt"]
                }
/*                Segment {
                    icon: "\uf1eb"
                    tooltip: "Wi-Fi / Network"
                    onClicked: networkManagerProc.running = true
                }
                Process {
                    id: networkManagerProc
                    command: ["bash", "-c", "networkmanager_dmenu"]
                }
                ...
                Segment {
                    ...
                    onClicked: {
                        wallpaperProc.command = ["bash", "-c", "$HOME/.config/waybar/change_wallpaper.sh"]
                        wallpaperProc.running = true
                    }
                    onRightClicked: {
                        wallpaperProc.command = ["bash", "-c", "$HOME/.config/waybar/change_wallpaper.sh special"]
                        wallpaperProc.running = true
                    }
                }
        ...
        PopupWindow {
            ...
            ColumnLayout {
                ...
                anchors.margins: 4
                spacing: 2
                Segment { label: "Shutdown";  onClicked: { Quickshell.execDetached(["systemctl", "poweroff"]);  powerMenu.visible = false } }
                Segment { label: "Reboot";    onClicked: { Quickshell.execDetached(["systemctl", "reboot"]);    powerMenu.visible = false } }
                Segment { label: "Suspend";   onClicked: { Quickshell.execDetached(["systemctl", "suspend"]);   powerMenu.visible = false } }
                Segment { label: "Hibernate"; onClicked: { Quickshell.execDetached(["systemctl", "hibernate"]); powerMenu.visible = false } }
            }
        }
    }
}
